 ...
 *
 * Key changes vs the published waveshare/esp_lcd_hx8394 registry releases:
 *   • 4-argument API using esp_lcd_panel_hx8394_config_t.
 *   • panel_dev_config->rgb_ele_order used instead of removed ->color_space.
...
/* DSI lane-count command values sent to the panel */
...
typedef struct {
    ...
    /* Saved originals from the underlying MIPI DPI panel */
...
/* Default panel initialisation sequence (used when hx_config->init_cmds is NULL) */
static const hx8394_lcd_init_cmd_t s_vendor_init_default[] = {
    /* {cmd, {data...}, data_bytes, delay_ms} */
    {0xB9, (const uint8_t[]){0xFF, 0x83, 0x94}, 3, 0},
    {0xB1, (const uint8_t[]){0x48, 0x0A, 0x6A, 0x09, 0x33, 0x54, 0x71, 0x71, 0x2E, 0x45}, 10, 0},
    {0xBA, (const uint8_t[]){0x61, 0x03, 0x68, 0x6B, 0xB2, 0xC0}, 6, 0},
    {0xB2, (const uint8_t[]){0x00, 0x80, 0x64, 0x0C, 0x06, 0x2F}, 6, 0},
    {0xB4, (const uint8_t[]){0x1C, 0x78, 0x1C, 0x78, 0x1C, 0x78, 0x01, 0x0C, 0x86,
                              0x75, 0x00, 0x3F, 0x1C, 0x78, 0x1C, 0x78, 0x1C, 0x78,
                              0x01, 0x0C, 0x86}, 21, 0},
    {0xD3, (const uint8_t[]){0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x08, 0x08, 0x32,
    ...
    {0xD5, (const uint8_t[]){0x18, 0x18, 0x18, 0x18, 0x22, 0x23, 0x20, 0x21, 0x04,
                              0x05, 0x06, 0x07, 0x00, 0x01, 0x02, 0x03, 0x18, 0x18,
                              0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18,
                              0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18,
                              0x18, 0x18, 0x18, 0x18, 0x19, 0x19, 0x19, 0x19}, 44, 0},
    {0xD6, (const uint8_t[]){0x18, 0x18, 0x19, 0x19, 0x21, 0x20, 0x23, 0x22, 0x03,
                              0x02, 0x01, 0x00, 0x07, 0x06, 0x05, 0x04, 0x18, 0x18,
                              0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18,
                              0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18,
                              0x18, 0x18, 0x18, 0x18, 0x19, 0x19, 0x18, 0x18}, 44, 0},
    {0xE0, (const uint8_t[]){0x07, 0x08, 0x09, 0x0D, 0x10, 0x14, 0x16, 0x13, 0x24,
    ...
    {0xCC, (const uint8_t[]){0x0B}, 1, 0},
    {0xC0, (const uint8_t[]){0x1F, 0x73}, 2, 0},
    {0xB6, (const uint8_t[]){0x6B, 0x6B}, 2, 0},
    {0xD4, (const uint8_t[]){0x02}, 1, 0},
    {0xBD, (const uint8_t[]){0x01}, 1, 0},
    {0xB1, (const uint8_t[]){0x00}, 1, 0},
    {0xBD, (const uint8_t[]){0x00}, 1, 0},
    {0xBF, (const uint8_t[]){0x40, 0x81, 0x50, 0x00, 0x1A, 0xFC, 0x01}, 7, 0},
    {0x3A, (const uint8_t[]){0x50}, 1, 0},
    {0x11, (const uint8_t[]){0x00}, 0, 200},
    {0xB2, (const uint8_t[]){0x00, 0x80, 0x64, 0x0C, 0x06, 0x2F, 0x00, 0x00,
                              0x00, 0x00, 0xC0, 0x18}, 12, 0},
    {0x29, (const uint8_t[]){0x00}, 0, 80},
};
                                    ...
                                    esp_lcd_panel_handle_t                   *ret_panel)
{
    ESP_LOGI(TAG, "version: %d.%d.%d",
             ESP_LCD_HX8394_VER_MAJOR, ESP_LCD_HX8394_VER_MINOR, ESP_LCD_HX8394_VER_PATCH);

    ESP_RETURN_ON_FALSE(io && panel_dev_config && hx_config && ret_panel,
                        ESP_ERR_INVALID_ARG, TAG, "invalid argument");
    ESP_RETURN_ON_FALSE(hx_config->dsi_bus && hx_config->dpi_config,
                        ESP_ERR_INVALID_ARG, TAG, "dsi_bus and dpi_config are required");

    esp_err_t ret = ESP_OK;
    hx8394_panel_t *hx8394 = calloc(1, sizeof(hx8394_panel_t));
    ...
    /* RGB element order (replaces removed color_space field) */
    ...
    /* Bits-per-pixel → COLMOD value */
    switch (panel_dev_config->bits_per_pixel) {
    case 16: hx8394->colmod_val = 0x55; break;  /* RGB565 */
    case 18: hx8394->colmod_val = 0x66; break;  /* RGB666 */
    case 24: hx8394->colmod_val = 0x77; break;  /* RGB888 */
    default:
        ESP_GOTO_ON_FALSE(false, ESP_ERR_NOT_SUPPORTED, err, TAG,
    ...
    hx8394->init_cmds      = hx_config->init_cmds;
    hx8394->init_cmds_size = hx_config->init_cmds_size;
    /* num_data_lanes == 0 defaults to 2 lanes (matches the case-0 branch below) */
    hx8394->lane_num       = hx_config->num_data_lanes;

    /* Create the underlying MIPI DPI panel */
    esp_lcd_panel_handle_t panel_handle = NULL;
    ESP_GOTO_ON_ERROR(
        esp_lcd_new_panel_dpi(hx_config->dsi_bus, hx_config->dpi_config, &panel_handle),
        err, TAG, "create MIPI DPI panel failed");
    ESP_LOGD(TAG, "new MIPI DPI panel @%p", panel_handle);

    /* Save originals, then override with HX8394-specific ops */
...
static esp_err_t panel_hx8394_del(esp_lcd_panel_t *panel)
{
    hx8394_panel_t *hx8394 = panel->user_data;
...
static esp_err_t panel_hx8394_init(esp_lcd_panel_t *panel)
{
    ...
    /* DSI lane-count command */
    uint8_t lane_command;
    switch (hx8394->lane_num) {
    case 0: /* fall-through – default to 2 lanes */
    case 2: lane_command = HX8394_DSI_2_LANE; break;
    case 1: lane_command = HX8394_DSI_1_LANE; break;
    case 3: lane_command = HX8394_DSI_3_LANE; break;
    case 4: lane_command = HX8394_DSI_4_LANE; break;
    default:
    ...
    ESP_RETURN_ON_ERROR(
        esp_lcd_panel_io_tx_param(io, HX8394_CMD_DSI_INT0,
                                  (uint8_t[]){lane_command}, 1),
        TAG, "send DSI_INT0 failed");
    ...
    if (hx8394->init_cmds) {
        ...
        init_cmds      = s_vendor_init_default;
        init_cmds_size = sizeof(s_vendor_init_default) / sizeof(hx8394_lcd_init_cmd_t);
    }
    ...
    for (int i = 0; i < init_cmds_size; i++) {
        if (init_cmds[i].data_bytes > 0) {
            ...
            case LCD_CMD_MADCTL:
                ...
                hx8394->madctl_val = ((const uint8_t *)init_cmds[i].data)[0];
                break;
            case LCD_CMD_COLMOD:
                is_cmd_overwritten = true;
                hx8394->colmod_val = ((const uint8_t *)init_cmds[i].data)[0];
                break;
            default:
            ...
            if (is_cmd_overwritten) {
                is_cmd_overwritten = false;
                ESP_LOGW(TAG, "Command 0x%02Xh will be overwritten by custom init sequence",
                         init_cmds[i].cmd);
            }
        }
        ESP_RETURN_ON_ERROR(
